...
resource "oci_core_security_list" "app_seclist" {
  compartment_id = var.compartment_ocid
  vcn_id         = oci_core_virtual_network.vcn.id
  display_name   = "Private App Security List (frontend + backend)"

  # LB to frontend (nginx)
  ingress_security_rules {
    protocol  = local.tcp
...
resource "oci_core_security_list" "db_seclist" {
  ...
  # ops -> all container DBs (for schema management via Liquibase / mongosh)
  ingress_security_rules {
    protocol  = local.tcp
    source    = oci_core_subnet.public_subnet.cidr_block
    stateless = false
