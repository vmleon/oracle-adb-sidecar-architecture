...
# Upload each policy markdown as a Terraform-managed object. This puts the
# objects in state, so `terraform destroy` removes them before the bucket
# (no 409-BucketNotEmpty), and content changes trigger re-upload on apply.
resource "oci_objectstorage_object" "banking_rag_docs" {
  for_each  = fileset("${path.module}/../../database/banking-policy-docs", "*.md")
  bucket    = oci_objectstorage_bucket.banking_rag_docs.name
  namespace = oci_objectstorage_bucket.banking_rag_docs.namespace
  object    = each.value
  source    = "${path.module}/../../database/banking-policy-docs/${each.value}"
}
...
resource "oci_objectstorage_object" "ansible_frontend_artifact_object" {
  bucket      = oci_objectstorage_bucket.artifacts_bucket.name
  source      = data.archive_file.ansible_frontend_artifact.output_path
  namespace   = data.oci_objectstorage_namespace.objectstorage_namespace.namespace
  object      = "ansible_frontend_artifact.zip"
  content_md5 = data.archive_file.ansible_frontend_artifact.output_md5
}

resource "oci_objectstorage_preauthrequest" "ansible_frontend_artifact_par" {
  namespace    = data.oci_objectstorage_namespace.objectstorage_namespace.namespace
  bucket       = oci_objectstorage_bucket.artifacts_bucket.name
  name         = "ansible_frontend_artifact_par"
  access_type  = "ObjectRead"
  object_name  = oci_objectstorage_object.ansible_frontend_artifact_object.object
  time_expires = timeadd(time_static.deploy_time.rfc3339, "${var.artifacts_par_expiration_in_days * 24}h")
}
...
resource "oci_objectstorage_object" "front_artifact_object" {
  bucket      = oci_objectstorage_bucket.artifacts_bucket.name
  source      = data.archive_file.frontend_artifact.output_path
  namespace   = data.oci_objectstorage_namespace.objectstorage_namespace.namespace
  object      = "frontend_artifact.zip"
  content_md5 = data.archive_file.frontend_artifact.output_md5
}

resource "oci_objectstorage_preauthrequest" "frontend_artifact_par" {
  namespace    = data.oci_objectstorage_namespace.objectstorage_namespace.namespace
  bucket       = oci_objectstorage_bucket.artifacts_bucket.name
  name         = "frontend_artifact_par"
  access_type  = "ObjectRead"
