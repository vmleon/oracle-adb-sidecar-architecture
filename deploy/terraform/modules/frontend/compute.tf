locals {
  cloud_init_content = templatefile("${path.module}/userdata/bootstrap.tftpl", {
    ...
    ansible_frontend_par_full_path = var.ansible_frontend_artifact_par_full_path
    frontend_par_full_path         = var.frontend_artifact_par_full_path
...
resource "oci_core_instance" "instance" {
  ...
  compartment_id      = var.compartment_ocid
  display_name        = "frontend${var.project_name}${var.deploy_id}"
  shape               = var.instance_shape
  ...
  create_vnic_details {
    ...
    hostname_label            = "frontend${var.deploy_id}"
  }
